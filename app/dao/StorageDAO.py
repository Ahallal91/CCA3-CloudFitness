...
class StorageDAO:
    ...
    def upload_image(self, image, image_name, bucket):
    ...
    def format_image_name(self, image):
    ...
    def upload_exercise_image(self, image, bucket):
        filename_with_uuid = self.format_image_name(image)  # add uuid to end of filename to ensure uniqueness
        if not self.upload_image(image, filename_with_uuid, bucket):
            return ImageUploadFailed

        return filename_with_uuid

    def get_url_for(self, bucket, region, folder, image_name):
